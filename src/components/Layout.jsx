    ...
    <div className="min-h-screen flex bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100 transition-colors duration-200">
      ...
      <aside className="hidden md:flex flex-col w-64 bg-slate-900 border-r border-slate-800 shrink-0">
        ...
        <div className="p-4 border-t border-slate-800 space-y-2">
          <div className="px-4 text-[10px] text-slate-500 truncate" title={session?.username}>
            Oturum: {session?.username || 'Bilinmeyen'}
          </div>
          <button
            onClick={onLogout}
            className="w-full flex items-center gap-3 px-4 py-2 rounded-xl text-sm font-semibold text-rose-400 hover:bg-rose-500/10 transition-all cursor-pointer"
          >
            <LogOut size={18} />
            <span>Çıkış Yap</span>
          </button>
      ...
      {mobileMenuOpen && (
        <div className="fixed inset-0 z-50 flex md:hidden bg-slate-900/60 backdrop-blur-sm">
          <div className="w-64 bg-slate-900 h-full p-5 flex flex-col shadow-2xl animate-in slide-in-from-left duration-200">
            ...
            <div className="px-4 text-[10px] text-slate-500 truncate mb-2">
              Oturum: {session?.username || 'Bilinmeyen'}
            </div>
            <button
              onClick={onLogout}
              className="mt-auto w-full flex items-center gap-3 px-4 py-2 rounded-xl text-sm font-semibold text-rose-400 hover:bg-rose-500/10 cursor-pointer"
            >
              <LogOut size={18} />
              <span>Çıkış Yap</span>
            </button>
